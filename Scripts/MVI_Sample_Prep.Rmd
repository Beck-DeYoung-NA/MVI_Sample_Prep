...
We initially set bypass_missing_folders = F because we want to make sure every market we expend is there. If ICC or IDC are the only ones that are missing, then you are okay to set it to TRUE since we have expected those to be missing in the past few quarters. If OPs misnamed a folder in the `Files_From_CMS` directory. You should update the `Country_Info` sheet in the template so that the `Country_Name_CMS` variable is aligned with the folder name they used.

```{r "Load in the data"}
MVI_Sample <- load_MVI_raw_data(bypass_missing_folders = FALSE) 
...
These are used for diagnostics and sent to OPS and PM later on
 ...
 - Scan though them and make sure the sample requested vs sample received is not drastically different (>30%). Or at least make sure the difference is at least consistent across all cells in the market.
...
I have only ever seen the tenure flagged in Belgium (Country 41), and it usually somewhere between 100-800 people. These removals are normal. If anyone from a different market is flagged, make sure to call it out in the email.
...
```{r Load in PCT}
# Load in pct list
aif_pct <- read_excel(PCT_LIST_PATH) %>% 
  set_names(c("NA_Prd_Code",	"Cell_Number",	"Product_Code",	"Card_ProductAIF",	"COUNTRY", "NA_Country_Code",	"Count",	"Flag")) %>% 
  mutate(Formatted_PCT_Code = substr(Product_Code, 1, 3), # Some PCT Codes come in badly formatted with a bunch of extra characters
         ...
         NA_Country_Code = as.numeric(NA_Country_Code)) %>% select(-Count, -Flag) %>% 
  distinct()
```
...
to_remove_amex_id_dupes_summary <- to_remove_amex_id_dupes %>% ungroup()
...
# load(f_str("../Temporary_Data/{MVIQ}_Checking_{last_save_date}.Rdata"))
```
...
Now that you are done, remember to Knit to html -- this will rerun the code and create a log of everything.