...
```
This was performing the weighting segmentation based on a sheet called MVI Q1'23 Table. But this was an extra step previously, so instead of making these sheet, we use the original weighting sheet and perform a few more steps in R above
```{r Weighting Segments2}
# Weighting Segments
# weighting_segments <- read_excel(HELPER_FILE_PATH, sheet = "Weighting_Segments") %>% 
...
MVI_Sample_ws <- MVI_Sample_ws %>% 
  mutate(across(c("Establishment_Date"), ~str_replace_all(., "-", "/")), # Some date are different format
         ...
         SUBJECT_LINE_INSERTION =  gsub("[\u00A0\\h\\v]+$", "", SUBJECT_LINE_INSERTION) # Remove non-breaking spaces
         )
```
