...
country_codes <- read_excel(HELPER_FILE_PATH, sheet = "Country_Info") %>% # Country and language codes
  set_names(c("NA_Country_Code",	"Country",	"Country_Language",	"NA_Language_Index",
              "FileExt",	"Language_Code",	"NA_Language_Code",	"CV_ICS_Region",	"Filename")) 
...
reporting_names <- read_excel(HELPER_FILE_PATH,
                sheet = "CV_Reporting_Names") %>% 
...
#### **STOP HERE: SEND THE THREE CREATED FILES TO OPS** {style="color: red"}
...
#### "Here are summary statistics of the people that were removed for low tenure, which is <12 in Australia and New Zealand and <6 elswhere. There were X people removed." Replace X with the number of removals.
-   `Files_to_send/Removed_Tenure_Summary.csv`

If they want indidivual people with tenure removed, that is located in 
-   `Diagnostic_Files/Removed_Tenure.csv`
-   This file will be flagged by IT's outlook sensitive info monitor, so instead of sending it in an email, you should copy the file to the following location and tell the PM where it is:
  - `\\pm1\33-626\Quantitative\Sampling-Weighting\Communications\QUARTER`
...
"Attached in the summary file are a the PCT Codes from the PML that do not line up with the AIF (based on country code, cell number, and NA product code). 
...
The non-summary file has the individual customers that are misaligned."

-   `Files_to_Send/PCT_not_in_AIF.csv`
-   `Files_to_Send/PCT_not_in_AIF_Summary.csv`
...
amex_id_dups <- MVI_Sample %>% 
  group_by(Amex_Customer_ID) %>% # CHANGED TO BE ACCROSS COUNTRY
  filter(n() > 1) 

amex_id_dups_within_country <- MVI_Sample %>% 
  group_by(NA_Country_Code, Amex_Customer_ID) %>% # CHANGED TO BE ACCROSS COUNTRY
  filter(n() > 1) 
...
if (n_dups_across > 0){
  ...
  amex_id_dups_across_country %>% 
    select(NA_Country_Code, NA_Language_Index, FileExt, Cell_Number, Amex_Customer_ID, Unique_Record_Identifier15, Tenure, Total_OD_Spend_USD) %>% 
...
# COULD BE WE WANT THE ONES THAT DONT HAVE ENOUGH SAMPLE
 ...
 MVI_Sample_no_dups <- MVI_Sample_no_dups %>% 
  ...
  filter(row_number() == 1) %>% ungroup() %>% 
  # Reset ordering
  arrange(NA_Country_Code, NA_Language_Index, FileExt, Cell_Number)
```
...
member_since_flag %>% 
  bind_rows(dob_flag) %>% 
  select(NA_Country_Code, NA_Language_Index, FileExt, Amex_Customer_ID, Unique_Record_Identifier15, Member_Since, dYOB, Tenure) %>% 
  write_csv(f_str("../Files_to_send/Member_Since_DOB_flags_{MVIQ}.csv"), na = "")
```

### STOP HERE: SEND FILES TO OPS {style="color: red"}

"Attached is the list of people who were removed for having unlikely dates of birth or member_since values. Please confirm these are okay to be deleted"
-   `Files_to_send/Member_Since_DOB_flags.csv`
