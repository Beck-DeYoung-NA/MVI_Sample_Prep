...
MVI_Sample <- map(countries, load_raw_data) %>%
  reduce(function(x, y) {
    if (join_counter == 0) message("--------------Joining all markets--------------")
...
#   #select(Update_NA_Cell,Update_Product,	Update_PCT_Code) %>%
#   mutate(Update_NA_Cell_Number = as.numeric(Update_NA_Cell_Number),
#     Update_Cell = str_pad(Update_NA_Cell_Number, 2, 'left', '0'))
# 
# MVI_Sample <- MVI_Sample %>%
#   left_join(pct_changes) %>% 
#   mutate(Cell_Number = if_else(!is.na(Update_Cell), Update_Cell, Cell_Number),
#          NA_Cell_Number = if_else(!is.na(Update_NA_Cell_Number), Update_NA_Cell_Number, NA_Cell_Number),
...
#   select(-starts_with("Update"), -NA_Product_Code)
```
...
addWorksheet(Diagnostic_WB, "Market_Level_Summary")
writeData(Diagnostic_WB, "Market_Level_Summary", mrkt_level_summary)
```
...
to_remove_tenure_summary <- to_remove_tenure %>% 
  group_by(NA_Country_Code, NA_Language_Index, File_Ext) %>% 
...
to_remove_tenure_summary_w_tenure <- to_remove_tenure %>% 
  group_by(NA_Country_Code, NA_Language_Index, File_Ext, Tenure) %>% 
  summarize(Tenure_Removed = n()) %>% 
...
if (nrow(to_remove_tenure) > 0){
  warning(f_str("There were {nrow(to_remove_tenure)} observations removed with erroneous tenure. These are found in the Diagnostic_Files/Removed_Tenure.csv file"))
  
  # Write to excel to eventually send to OPS Manager
  addWorksheet(Diagnostic_WB, "Removed_Tenure_Individuals")
  writeData(Diagnostic_WB, "Removed_Tenure_Individuals", to_remove_tenure)
  
  addWorksheet(Diagnostic_WB, "Removed_Tenure_Summary")
  writeData(Diagnostic_WB, "Removed_Tenure_Summary", to_remove_tenure_summary_w_tenure)
...
misaligned_pct_codes_sum <- misaligned_pct_codes_sum %>% 
  ...
  select(NA_Country_Code, Cell_Number, NA_Language_Index,
...
But, instead of attaching files, copy the file to the `\\pm1\33-626\Quantitative\Sampling-Weighting\Communications\QUARTER` folder and send the link. Here is an example email.
    ...
    Here is a diagnostic file with some initial findings from the data for you to review. Please let us know what you think: `FILE`.
...
writeData(Diagnostic_WB, "Rem_AMEXID_Dupes_Inds", to_remove_amex_id_dupes)

addWorksheet(Diagnostic_WB, "Rem_AMEXID_Dupes_Summary")
writeData(Diagnostic_WB, "Rem_AMEXID_Dupes_Summary", to_remove_amex_id_dupes_summary)
```
...
to_remove_ms_summary <- member_since_flag  %>% 
  group_by(NA_Country_Code, NA_Language_Index, File_Ext) %>% 
  summarize(Member_Since = n())

to_remove_dob_summary <- dob_flag %>% 
  group_by(NA_Country_Code, NA_Language_Index, File_Ext) %>% 
  summarize(YOB = n())

to_remove_ms_dob <- member_since_flag %>% 
  bind_rows(dob_flag) %>% 
...
writeData(Diagnostic_WB, "Removed_Member_Since_DOB", to_remove_ms_dob)
```
...
missing_spend %>% write_csv(f_str("../Diagnostic_Files/{MVIQ}_Missing.csv"), na = "")
zero_spend %>% write_csv(f_str("../Diagnostic_Files/{MVIQ}_Zero_Spend.csv"), na = "")
neg_spend %>% write_csv(f_str("../Diagnostic_Files/{MVIQ}_Negative_Spend.csv"), na = "")
```
...
mrkt_level_summary <- mrkt_level_summary %>% 
  ...
  left_join(to_remove_dob_summary) %>% 
  left_join(to_remove_ms_summary) %>% 
  left_join(to_remove_tenure_summary) %>% 
  left_join(to_remove_pct_codes_summary) %>%
  left_join(to_remove_amex_id_dupes_summary) %>% 
