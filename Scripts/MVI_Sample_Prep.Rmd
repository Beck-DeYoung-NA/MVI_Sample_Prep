...
NOTE YOU WILL ONLY KNOW IF YOU NEED TO DO THIS AFTER YOU SEND THE FIRST DIAGNOSTIC EMAIL HALFWAY DOWN THE SCRIPT. 
 ...
 - This occurred after we identified misalignment, the code had to be adjusted up here so that we extracted the correct subject line inserts and card product names with the adjustments. This has only happened once in 2023 (Q4)
 - Unfortunately, we cannot say for certain how OPS will provide the changes, so you may need to change the loading in the data step if this occurs
  - The information you need is the Unique_Record_Identifier15, Update_NA_Cell,Update_Product,	Update_NA_Product_Code for all people to update.
 
```{r}
# pct_changes <- read_excel(HELPER_FILE_PATH, 'PCT_Code_Changes') %>% 
#   select(Unique_Record_Identifier15, Update_NA_Cell,Update_Product,	Update_NA_Product_Code) %>% 
...
saveWorkbook(Diagnostic_WB, file = f_str("../Files_to_send/MVI_Diagnostics_{MVIQ}.xlsx"), overwrite = TRUE)
```
...
#### **When PM gets back on which to remove, update the next chunk**
  ...
  - If they say you can remove all samples then just procede with running the next chunk
...
save(MVI_final_postals, file="../Temporary_Data/MVI_Postals.Rdata")
save(MVI_final, file="../Temporary_Data/MVI_Final_Checking_{Sys.Date()}.Rdata")
```
...
# This includes extra variables to allow for checking
         ...
         # COB,  
         # Filename
MVI_final %>% write_csv(f_str("../All_Sample_Files/MVI_Final_Checking_{MVIQ}_{Sys.Date()}.csv"), na = "")
```
...
# load("../Temporary_Data/MVI_Final_Checking_{last_save_date}.Rdata")
```
...
# load("../Temporary_Data/MVI_Postals.Rdata")
